...
export async function resolveBrowserExecutor(
  ...
  if (!provider) {
    throw new Error(
      `Unsupported browser model: ${options.model}. Use a GPT, Gemini, Claude, or Grok model.`,
    );
  }
  if (remote) {
  ...
  if (provider === "claude" || provider === "grok") {
    const { createProviderWebExecutor } = await import("../provider-web/index.js");
    return createProviderWebExecutor();
  }
